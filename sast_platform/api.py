...
def create_platform() -> Platform:
    ...
    repos.users.add(admin, admin.user_id)
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            try:
                ...
                if self.path == "/api/rules": return self.send_json(200, platform.repos.rules.all())
                if self.path.startswith("/api/projects/"):
                    parts = self.path.split("/")
                    project = platform.require_project(user, parts[3])
                    if len(parts) == 5 and parts[4] == "runs": return self.send_json(200, platform.repos.project_runs(project.project_id))
                    if len(parts) == 7 and parts[4] == "runs" and parts[6] == "findings": return self.send_json(200, platform.repos.run_findings(parts[5]))
